...
function ckeditor_set() {
   ...
   $record = array('editor' => 'ckeditor', 'format' => 'filtered_html', 'settings' => $settings);
   $res1 = drupal_write_record('wysiwyg', $record);
   $record = array('editor' => 'ckeditor', 'format' => 'full_html', 'settings' => $settings);
   $res2 = drupal_write_record('wysiwyg', $record);
   if ($res1 && $res2) {
     print "Set wysiwyg settings sucess\n";
   } else {
     print " failure\n";
   }
}
